...
void extra_inspect(T)(T thing, uint indent, string name) {
	if(indent == 0) {
		writeln("!!! inspect extra:");
	}

	static if(is(T == const Token)) {
		TokenData data = get_token_data(thing);
		...
		writefln("%s!!! extra_inspect(T) override needed for: %s", pad(indent), thing);
	}
}
...
void inspect(const ASTNode node, size_t indent, string name) {
	...
	if(auto addExp = cast(const AddExpression) node) {
		...
		addExp.operator.extra_inspect(indent, "operator");
		...
		attr.attribute.extra_inspect(indent, "attribute");
		...
		assExp.operator.extra_inspect(indent, "operator");
		...
		atTrr.identifier.extra_inspect(indent, "identifier");
		...
		foreach(iden; autoDec.identifiers) {
			iden.extra_inspect(indent, "identifiers[]");
		}
		...
		declar.name.extra_inspect(indent, "name");
		...
		equalExp.operator.extra_inspect(indent, "operator");
		...
		forEachType.identifier.extra_inspect(indent, "identifier");
		...
		funcDec.name.extra_inspect(indent, "name");
		...
		idenTemp.identifier.extra_inspect(indent, "identifier");
		...
		ifStatement.identifier.extra_inspect(indent, "identifier");
		...
		mulExp.operator.extra_inspect(indent, "operator");
		...
		priExp.dot.extra_inspect(indent, "dot");
		priExp.primary.extra_inspect(indent, "primary");
		priExp.identifierOrTemplateInstance.inspect(indent, "identifierOrTemplateInstance");
		priExp.basicType.extra_inspect(indent, "basicType");
		...
		relExp.operator.extra_inspect(indent, "operator");
		...
		shiftExp.operator.extra_inspect(indent, "operator");
	} else if(auto singImpo = cast(const SingleImport) node) {
		singImpo.rename.extra_inspect(indent, "rename");
		...
		storageClass.token.extra_inspect(indent, "token");
		...
		tempInst.identifier.extra_inspect(indent, "identifier");
		...
		astType2.builtinType.extra_inspect(indent, "builtinType");
